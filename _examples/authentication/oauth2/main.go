...
// can be used with ion but at this example we will see the markbates' goth:
//
...
func BeginAuthHandler(ctx context.Context) {
	...
	ctx.Redirect(url, ion.StatusTemporaryRedirect)
}
...
func main() {
	...
	app.Get("/logout/{provider}", func(ctx context.Context) {
		Logout(ctx)
		ctx.Redirect("/", ion.StatusTemporaryRedirect)
	})
	...
	// http://localhost:3000
	app.Run(ion.Addr("localhost:3000"))
}
