...
func main() {
	...
	// create a custom path reverser, ion let you define your own host and scheme
	// which is useful when you have nginx or caddy in front of ion.
	rv := router.NewRoutePathReverser(app, router.WithHost(host), router.WithScheme("http"))
	// locate and define our templates as usual.
	templates := ion.HTML("./templates", ".html")
